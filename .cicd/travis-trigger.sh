...
[[ -z $TRAVIS_TRIGGER_API_KEY ]] && echo '+++ :no_entry: ERROR: TRAVIS_TRIGGER_API_KEY key not found!' && exit 1
...
export RESULT="$(curl -s -X POST -H "Content-Type: application/json" -H "Accept: application/json" -H "Travis-API-Version: 3" -H "Authorization: token $TRAVIS_TRIGGER_API_KEY" -d "$BODY" https://api.travis-ci.org/repo/EOSIO%2f${SOURCE_SLUG}/requests)"
echo 'API Response:'
