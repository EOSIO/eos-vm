...
#define EOS_VM_ASSERT(expr, err_type) if (!UNLIKELY(expr)) { return err_type; }

// assert but invalidate the enclosing class
#define EOS_VM_ASSERT_INVALIDATE( expr, err_type ) if (_valid = false; !UNLIKELY(expr)) { return err_type; }
